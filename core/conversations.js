...
class Conversation extends Chat {
	...
	respond(payload, data) {
		...
		if (data.type === "postback" || data.type === "quick_reply") {
			...
			const specificPostbackCallback = this.listeningCallbacks.find(
				(callbackObject) =>
					callbackObject.event === `${data.type}:${postbackPayload}`
			);
			if (
				specificPostbackCallback &&
				typeof specificPostbackCallback.callback === "function"
			) {
				this.stopWaitingForAnswer();
				return specificPostbackCallback.callback.apply(this, [
					payload,
					this,
					data,
				]);
			...
			const genericPostbackCallback = this.listeningCallbacks.find(
				(callbackObject) => callbackObject.event === data.type
			);
			if (
				genericPostbackCallback &&
				typeof genericPostbackCallback.callback === "function"
			) {
				this.stopWaitingForAnswer();
				return genericPostbackCallback.callback.apply(this, [
					payload,
					this,
					data,
				]);
		...
		if (data.type === "attachment") {
			const attachmentCallback = this.listeningCallbacks.find(
				(callbackObject) => callbackObject.event === "attachment"
			);
			if (
				attachmentCallback &&
				typeof attachmentCallback.callback === "function"
			) {
				this.stopWaitingForAnswer();
		...
		if (data.type === "message" && patternCallbacks.length > 0) {
			for (let i = 0; i < patternCallbacks.length; i += 1) {
				//const match = textMatchesPatterns(
				//  payload.message.text,
				//  patternCallbacks[i].pattern
				//);
				const match = true;
				if (match !== false) {
					this.stopWaitingForAnswer();
					...
					return patternCallbacks[i].callback.apply(this, [
						payload,
						this,
						data,
					]);
		...
		if (
			...
			const quickReplyCallback = this.listeningCallbacks.find(
				(callbackObject) =>
					callbackObject.event === `quick_reply` ||
					callbackObject.event ===
						`quick_reply:${payload.message.quick_reply.payload}`
			);
			if (
				quickReplyCallback &&
				typeof quickReplyCallback.callback === "function"
			) {
				return;
			}
		}
